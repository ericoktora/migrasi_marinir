    ...
    try:
        ...
        query = """
            INSERT INTO kategori_alat (
                id_kategori_alat, nama, kode_lis, SN, grub_alat, id_user, status, created_at, updated_at
            ) VALUES (
                %(id_kategori_alat)s, %(nama)s, %(kode_lis)s, %(sn)s, %(grub_alat)s, %(id_user)s, %(status)s, %(created_at)s, %(updated_at)s
            )
        """
