    ...
    try:
        ...
        query = """
            INSERT INTO kode_lab (
                id_kode_lab, id_spesimen_snomed, id_sub_kategori, grub1, grub2, grub3, kodelab_global, nama, en, kode_loinc, loinc_satuan, type_loinc, code_system, 
                kode_tes, kd_lis, satuan, status, nilai_rujukan, metoda, `case`, kode_his, harga, keterangan, info,
                koma, min, max, tipe_hasil, flaging, metode_input, created_at, updated_at, nilai_default
            ) VALUES (
                %(id_kode_lab)s, %(id_spesimen_snomed)s, %(id_sub_kategori)s, %(grub1)s, %(grub2)s, %(grub3)s, %(kodelab_global)s, %(nama)s, 
                %(en)s, %(kode_loinc)s, %(loinc_satuan)s, %(type_loinc)s, %(code_system)s, %(kode_tes)s, %(kd_lis)s, %(satuan)s, %(status)s, 
                %(nilai_rujukan)s, %(metoda)s, %(case)s, %(kode_his)s, %(harga)s, %(keterangan)s,
                %(info)s, %(koma)s, %(min)s, %(max)s, %(tipe_hasil)s, %(flaging)s, %(metode_input)s, 
